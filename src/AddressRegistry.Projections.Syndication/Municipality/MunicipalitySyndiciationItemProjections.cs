namespace AddressRegistry.Projections.Syndication.Municipality
{
    ...
    public class MunicipalitySyndiciationItemProjections : AtomEntryProjectionHandlerModule<MunicipalityEvent, Municipality, SyndicationContext>
    {
        ...
        private static async Task AddSyndicationItemEntry(AtomEntry<Municipality> entry, SyndicationContext context, CancellationToken ct)
        {
            var municipalitySyndicationItem = new MunicipalitySyndicationItem
            {
                ...
                OfficialLanguages = entry.Content.OfficialLanguages,
            };

            UpdateNamesByGemeentenamen(municipalitySyndicationItem, entry.Content.MunicipalityNames);
