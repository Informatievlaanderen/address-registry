namespace AddressRegistry.Tests.BackOffice.Handlers
{
    ...
    public class WhenCorrectingAddressRetirement : BackOfficeHandlerTest
    {
        ...
        public WhenCorrectingAddressRetirement(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task GivenRequest_ThenPersistentLocalIdETagResponse()
        {
            ...
            ApproveAddress(streetNamePersistentLocalId,
                addressPersistentLocalId);

            RetireAddress(streetNamePersistentLocalId,
                addressPersistentLocalId);

            var sut = new AddressCorrectRetirementHandler(
            ...
            // Act
            var result = await sut.Handle(new AddressCorrectRetirementRequest()
            {
            ...
            // Assert
            var stream = await Container.Resolve<IStreamStore>().ReadStreamBackwards(new StreamId(new StreetNameStreamId(new StreetNamePersistentLocalId(streetNamePersistentLocalId))), 4, 1);
