namespace AddressRegistry.Tests.BackOffice.Lambda.WhenReaddress
{
    ...
    public class GivenSourceAddressHasNoPostalCode : BackOfficeLambdaTest
    {
        ...
        public async Task ThenTicketingErrorIsExpected()
        {
            ...
            var sut = new ReaddressLambdaHandler(
                ...
                MockExceptionIdempotentCommandHandler(() => new AddressHasNoPostalCodeException(addressPersistentLocalId)).Object,
            ...
            ticketing.Verify(x =>
                x.Error(
                    It.IsAny<Guid>(),
                    new TicketError(
                        $"Deze actie is niet toegestaan op adressen die geen postcode hebben: {puri}.",
                        "BronAdresIdPostcode"),
                    CancellationToken.None));
        }
    }
