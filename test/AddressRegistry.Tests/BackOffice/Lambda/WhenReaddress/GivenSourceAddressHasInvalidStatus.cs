namespace AddressRegistry.Tests.BackOffice.Lambda.WhenReaddress
{
    ...
    public class GivenSourceAddressHasInvalidStatus : BackOfficeLambdaTest
    {
        private readonly BackOfficeContext _fakeBackOfficeContext;

        public GivenSourceAddressHasInvalidStatus(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketingErrorIsExpected()
        {
            ...
            ticketing.Verify(x =>
                x.Error(
                    It.IsAny<Guid>(),
                    new TicketError(
                        $"Deze actie is enkel toegestaan op adressen met status 'voorgesteld' of 'inGebruik': {puri}.",
                        "BronAdresIdAfgekeurdGehistoreerd"),
                    CancellationToken.None));
        }
    }
}
