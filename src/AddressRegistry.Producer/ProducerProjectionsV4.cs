namespace AddressRegistry.Producer
{
    ...
    /// <summary>
    /// Difference with V3: we fixed the old Readdress events when batching them to produce a new StreetNameWasReaddressed event.
    /// </summary>
    [ConnectedProjectionName("Kafka producer V4")]
    [ConnectedProjectionDescription("Projectie die berichten naar de kafka broker stuurt.")]
    public class ProducerProjectionsV4 : ConnectedProjection<ProducerContext>
    {
        public const string AddressTopicKey = "AddressV4Topic";

        private readonly IProducer _producer;

        public ProducerProjectionsV4(
            IProducer producer,
            ...
            JsonSerializerSettings jsonSerializerSettings)
        {
            ...
            When<Be.Vlaanderen.Basisregisters.ProjectionHandling.SqlStreamStore.Envelope<StreetName.AddressPositionCrsWasChanged>>(async (_, message, ct) =>
            {
                await Produce(message.Message.StreetNamePersistentLocalId, message.Message.ToContract(), message.Position, ct);
            });
            ...
            When<Be.Vlaanderen.Basisregisters.ProjectionHandling.SqlStreamStore.Envelope<StreetName.AddressBoxNumbersWereCorrected>>(async (_, message, ct) =>
            {
                await Produce(message.Message.StreetNamePersistentLocalId, message.Message.ToContract(), message.Position, ct);
            });
            ...
            When<Be.Vlaanderen.Basisregisters.ProjectionHandling.SqlStreamStore.Envelope<StreetName.AddressHouseNumberWasReaddressed>>(async (_, message, ct) =>
            {
                ...
                var relevantMessages = streamPages
                    .SelectMany(x => x.Messages)
                    .Where(x => x.Type == StreetName.AddressHouseNumberWasReaddressed.EventName
                ...
                foreach (var streamMessage in relevantMessages.OrderBy(x => x.Position))
                {
                    ...
                    var @event = JsonConvert.DeserializeObject<StreetName.AddressHouseNumberWasReaddressed>(jsonData, jsonSerializerSettings);
