namespace AddressRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenAddressBackOfficeApproveRequest : AddressRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenAddressBackOfficeApproveRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sut = new ApproveSqsHandler(
                sqsQueue.Object,
                ticketingMock.Object,
                ticketingUrl,
                _backOfficeContext);

            var sqsRequest = new ApproveSqsRequest
            {
                Request = new BackOfficeApproveRequest
                {
        ...
        public void WithNoStreetNameFoundByAddressPersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                new ApproveSqsRequest
                {
                    Request = Fixture.Create<BackOfficeApproveRequest>()
                }, CancellationToken.None);
