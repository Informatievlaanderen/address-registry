namespace AddressRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenAddressBackOfficeRemoveRequest : AddressRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenAddressBackOfficeRemoveRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sut = new RemoveSqsHandler(
                sqsQueue.Object,
                ticketingMock.Object,
                ticketingUrl,
                _backOfficeContext);

            var sqsRequest = new RemoveSqsRequest
            {
                Request = new BackOfficeRemoveRequest
                {
        ...
        public void WithNoStreetNameFoundByAddressPersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                new RemoveSqsRequest
                {
                    Request = Fixture.Create<BackOfficeRemoveRequest>()
                }, CancellationToken.None);
