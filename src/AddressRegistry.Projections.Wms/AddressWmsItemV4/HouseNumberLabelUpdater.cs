namespace AddressRegistry.Projections.Wms.AddressWmsItemV4
{
    ...
    public sealed class HouseNumberLabelUpdater : IHouseNumberLabelUpdater
    {
        ...
        private async Task<IList<AddressWmsItemV4>> FindAddressesWithSharedPositionAndTheirBoxNumbers(
            WmsContext context,
            ...
            CancellationToken ct)
        {
            ...
            var localItems = context.AddressWmsItemsV4.Local.ToList();

            var dbItems = await context.AddressWmsItemsV4
                .Where(i => i.PositionX == position.X
                            && i.PositionY == position.Y
                            && i.AddressPersistentLocalId != addressPersistentLocalId
                            && i.Status == status
            ...
            // We need to verify that the AddressWmsItemV4 retrieved from the DB is not already in the local cache.
            ...
            // An example:
            //  context.AddressWmsItemsV4.Local (local cache) contains items: A, B, C and D
            ...
            var verifiedDbItems = new List<AddressWmsItemV4>();
        ...
        private static string? CalculateHouseNumberLabel(IEnumerable<AddressWmsItemV4> addresses)
        {
