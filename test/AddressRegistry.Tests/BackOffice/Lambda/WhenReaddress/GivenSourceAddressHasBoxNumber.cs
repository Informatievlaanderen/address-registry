namespace AddressRegistry.Tests.BackOffice.Lambda.WhenReaddress
{
    ...
    public class GivenSourceAddressHasBoxNumber : BackOfficeLambdaTest
    {
        private readonly BackOfficeContext _fakeBackOfficeContext;

        public GivenSourceAddressHasBoxNumber(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketingErrorIsExpected()
        {
            ...
            var sut = new ReaddressLambdaHandler(
                ...
                MockExceptionIdempotentCommandHandler(() => new AddressHasBoxNumberException(addressPersistentLocalId)).Object,
            ...
            ticketing.Verify(x =>
                x.Error(
                    It.IsAny<Guid>(),
                    new TicketError(
                        $"Deze actie is niet toegestaan op adressen met een busnummer: {puri}.",
                        "BronAdresIdBusnummer"),
                    CancellationToken.None));
        }
    }
}
