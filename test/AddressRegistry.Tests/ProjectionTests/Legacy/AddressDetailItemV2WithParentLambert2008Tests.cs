namespace AddressRegistry.Tests.ProjectionTests.Legacy
{
    ...
    /// <summary>
    /// The legacy projection stores the position as the EWKB it was given, so it holds whichever
    /// reference system the event store writes. These tests pin that down for Lambert 2008, which is
    /// what the event store will hold after its conversion. See ADR 0004.
    /// </summary>
    public class AddressDetailItemV2WithParentLambert2008Tests
        : AddressLegacyProjectionTest<AddressDetailProjectionsV2WithParent>
    {
