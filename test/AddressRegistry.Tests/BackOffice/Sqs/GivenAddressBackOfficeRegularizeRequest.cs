namespace AddressRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenAddressBackOfficeRegularizeRequest : AddressRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenAddressBackOfficeRegularizeRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sut = new RegularizeAddressSqsHandler(
                sqsQueue.Object,
                ticketingMock.Object,
                ticketingUrl,
                _backOfficeContext);

            var sqsRequest = new RegularizeAddressSqsRequest
            {
                Request = new RegularizeAddressBackOfficeRequest
                {
        ...
        public void WithNoStreetNameFoundByAddressPersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                new RegularizeAddressSqsRequest
                {
                    Request = Fixture.Create<RegularizeAddressBackOfficeRequest>()
                }, CancellationToken.None);
